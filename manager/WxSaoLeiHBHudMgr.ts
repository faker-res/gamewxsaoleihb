...
module gamewxsaoleihb.manager {
	export class WxSaoLeiHBHudMgr extends gamecomponent.managers.BaseMgr {
		static readonly EVENT_RETURN_MAPINFO: string = "WxSaoLeiHBHudMgr.RETURN_MAPINFO";
		private _data: any = [];
		constructor(game: Game) {
			super(game)
			this._delta = 3000;
			// this._game.network.addHanlder(Protocols.SMSG_WxSaoLeiHB_RETURN_MAPINFO, this, this.onOptHandler);
		}
		...
		private onOptHandler(optcode: number, msg: any): void {
			...
			this.event(WxSaoLeiHBHudMgr.EVENT_RETURN_MAPINFO);
		}
		 ...
		 */
		deltaUpdate() {
			this._game.network.call_WxSaoLeiHB_get_mapinfo()
		}

		clear() {
			Laya.timer.clearAll(this);
			// this._game.network.removeHanlder(Protocols.SMSG_WxSaoLeiHB_RETURN_MAPINFO, this, this.onOptHandler);
			super.clear();
		}
	}
}